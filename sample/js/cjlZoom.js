		...
		function _imageZoomPositioning(e){
			//get the percentage value of the location
				var perX = (e.pageX - $currentImg_offset.left) / $currentImg.outerWidth(), //.45
					...
					bgX = -( (perX * imageObject.width) - (defaults.size.width/2) ),
					bgY = -( (perY * imageObject.height) -  (defaults.size.height/2) );
		...
		var style = {
			lens : {
				...
				onmousemove: function(e){
					...
					$cjlZoom.css({
						top: e.pageY - (defaults.size.height/2),
						left : e.pageX - (defaults.size.width/2),
					});
					_imageZoomPositioning(e);
				}
			},
			...
			overlay : {
				init : function(e){
					...
					defaults.size.width = $currentImg.outerWidth();
					defaults.size.height = $currentImg.outerHeight();
					$cjlZoom.css({
